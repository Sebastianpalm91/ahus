<img src="https://media.giphy.com/media/ld28D1eniDy5W/giphy.gif" width="100%">

## Akademiska hus project
An issue reporting page for Akademiska Hus. Front end development with [react](https://reactjs.org/) and backend development with [laravel](https://laravel.com/docs/5.6/passport).

## Documentation
Only visually appealing in mobile view.

### Setup
- Download or clone the project
- Make sure you have [npm](https://docs.npmjs.com/) installed
- Navigate with the command prompt to the root file of the project
- Run commands:
```sh
  $ npm install
  $ npm start
```

## Development and Design by:
- Sebastian Palm
- Johan Sylvan
- Marcus Led
- Anders Berlander

## License

[MIT License](LICENSE)
